...
def main(args):
    ...
    print(f'Training took {training_end - start}s')
    ...
    images = pipe("a <cat-toy> in mad max fury road", num_images_per_prompt=2, num_inference_steps=25).images
    if not os.path.exists(results_path):
        os.mkdir(results_path)
    [image.save(f"{results_path}/{i}.jpeg") for i, image in enumerate(images)]

    print(f'Inference took {time() - training_end}s')
